...
func (c *connector) request() (*http.Response, *bufio.Reader, error) {
	...
	// 缓冲1, 超时退出后迟到的写入结果可安全落入, 避免发送协程永久阻塞泄漏
...
// 获取压缩拓展结果
// 服务端响应未包含的window_bits按RFC7692默认值15处理; 实际窗口位取客户端配置与服务端响应的较小值.
func (c *connector) getPermessageDeflate(extensions string) PermessageDeflate {
	...
	serverWindowBits := internal.SelectValue(serverPD.ServerMaxWindowBits == 0, 15, serverPD.ServerMaxWindowBits)
	clientWindowBits := internal.SelectValue(serverPD.ClientMaxWindowBits == 0, 15, serverPD.ClientMaxWindowBits)
	pd := PermessageDeflate{
		...
		ServerMaxWindowBits:   internal.Min(clientPD.ServerMaxWindowBits, serverWindowBits),
		ClientMaxWindowBits:   internal.Min(clientPD.ClientMaxWindowBits, clientWindowBits),
	}
...
func (c *connector) handshake() (*Conn, *http.Response, error) {
	...
	socket.randState.Store(internal.NextSeed())
