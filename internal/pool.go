package internal

import (
	"bytes"
	"math/bits"
	"sync"
)

type BufferPool struct {
	begin      int
	beginShift int
	shards     []*sync.Pool
}

// NewBufferPool 创建一个内存池
// left 和 right 表示内存池的区间范围，它们将被转换为 2 的 n 次幂
// 小于 left 的情况下，Get 方法将返回至少 left 字节的缓冲区；大于 right 的情况下，Put 方法不会回收缓冲区
func NewBufferPool(left, right uint32) *BufferPool {
	var begin, end = int(binaryCeil(left)), int(binaryCeil(right))
	var p = &BufferPool{
		begin:      begin,
		beginShift: bits.Len32(uint32(begin)) - 1,
	}
	for i := begin; i <= end; i *= 2 {
		capacity := i
		p.shards = append(p.shards, &sync.Pool{
			New: func() any { return bytes.NewBuffer(make([]byte, 0, capacity)) },
		})
	}
	return p
}

// Put 将缓冲区放回到内存池
func (p *BufferPool) Put(b *bytes.Buffer) {
	if b != nil {
		if pool := p.shard(b.Cap()); pool != nil {
			pool.Put(b)
		}
	}
}

// Get 从内存池中获取一个至少 n 字节的缓冲区
func (p *BufferPool) Get(n int) *bytes.Buffer {
	var size = Max(int(binaryCeil(uint32(n))), p.begin)
	if pool := p.shard(size); pool != nil {
		b := pool.Get().(*bytes.Buffer)
		if b.Cap() < size {
			b.Grow(size)
		}
		b.Reset()
		return b
	}
	return bytes.NewBuffer(make([]byte, 0, n))
}

func (p *BufferPool) shard(capacity int) *sync.Pool {
	if capacity <= 0 || capacity&(capacity-1) != 0 {
		return nil
	}
	var idx = bits.Len32(uint32(capacity)) - 1 - p.beginShift
	if idx < 0 || idx >= len(p.shards) {
		return nil
	}
	return p.shards[idx]
}

// binaryCeil 将给定的 uint32 值向上取整到最近的 2 的幂
//
//nolint:mnd // bit-smearing algorithm: shift values are inherent to the algorithm
func binaryCeil(v uint32) uint32 {
	v--
	v |= v >> 1
	v |= v >> 2
	v |= v >> 4
	v |= v >> 8
	v |= v >> 16
	v++
	return v
}

// NewPool 创建一个新的泛型内存池
func NewPool[T any](f func() T) *Pool[T] {
	return &Pool[T]{p: sync.Pool{New: func() any { return f() }}}
}

// Pool 泛型内存池
type Pool[T any] struct {
	p sync.Pool // 内嵌的 sync.Pool
}

// Put 将一个值放入池中
func (c *Pool[T]) Put(v T) {
	c.p.Put(v)
}

// Get 从池中获取一个值
func (c *Pool[T]) Get() T {
	return c.p.Get().(T)
}
